// Generated by CoffeeScript 1.4.0
(function() {
  var Funclet, async, bind, each, eachSeries, loglet, map, mapSeries, parallel, start,
    __slice = [].slice;

  async = require('async');

  loglet = require('loglet');

  Funclet = (function() {

    Funclet.make = function() {
      return new Funclet();
    };

    function Funclet() {
      this.calls = [];
      this.onError = console.error;
      this;

    }

    Funclet.prototype.bind = function() {
      var args, proc;
      proc = arguments[0], args = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
      return this.then(function(next) {
        return proc.apply(null, __slice.call(args).concat([next]));
      });
    };

    Funclet.prototype.then = function(proc) {
      this.calls.push(function() {
        var arg, cb, _i;
        arg = 2 <= arguments.length ? __slice.call(arguments, 0, _i = arguments.length - 1) : (_i = 0, []), cb = arguments[_i++];
        try {
          return proc.apply(null, __slice.call(arg).concat([cb]));
        } catch (e) {
          return cb(e);
        }
      });
      return this;
    };

    Funclet.prototype.map = function(ary, proc) {
      return this.then(function(next) {
        return async.map(ary, proc, next);
      });
    };

    Funclet.prototype.thenMap = function(proc) {
      return this.then(function(ary, next) {
        return async.map(ary, proc, next);
      });
    };

    Funclet.prototype.mapSeries = function(ary, proc) {
      return this.then(function(next) {
        return async.mapSeries(ary, proc, next);
      });
    };

    Funclet.prototype.thenMapSeries = function(proc) {
      return this.then(function(ary, next) {
        return async.mapSeries(ary, proc, next);
      });
    };

    Funclet.prototype.each = function(ary, proc) {
      return this.then(function(next) {
        return async.each(ary, proc, next);
      });
    };

    Funclet.prototype.thenEach = function(proc) {
      return this.then(function(ary, next) {
        return async.each(ary, proc, next);
      });
    };

    Funclet.prototype.eachSeries = function(ary, proc) {
      return this.then(function(next) {
        return async.eachSeries(ary, proc, next);
      });
    };

    Funclet.prototype.thenEachSeries = function(proc) {
      return this.then(function(ary, next) {
        return async.eachSeries(ary, proc, next);
      });
    };

    Funclet.prototype.parallel = function(tasks) {
      return this.then(function(next) {
        return async.parallel(tasks, next);
      });
    };

    Funclet.prototype.thenParallel = function() {
      return this.then(function(tasks, next) {
        return async.parallel(tasks, next);
      });
    };

    Funclet.prototype["catch"] = function(onError) {
      this.onError = onError;
      return this;
    };

    Funclet.prototype.done = function(lastCB) {
      var helper, interim, self;
      if (lastCB == null) {
        lastCB = function() {};
      }
      self = this;
      interim = [];
      helper = function(func, next) {
        var cb;
        cb = function() {
          var err, res;
          err = arguments[0], res = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
          loglet.debug('funclet.done.helper.cb', err, res);
          if (err) {
            return next(err);
          } else {
            interim = res;
            return next(null);
          }
        };
        try {
          loglet.debug.apply(loglet, ['funclet.done.helper'].concat(__slice.call(interim), [cb]));
          return func.apply(null, __slice.call(interim).concat([cb]));
        } catch (e) {
          return cb(e);
        }
      };
      async.eachSeries(this.calls, helper, function(err) {
        try {
          if (err) {
            return self.onError(err);
          } else {
            try {
              return lastCB.apply(null, interim);
            } catch (e2) {
              return self.onError(e2);
            }
          }
        } catch (e) {
          return self.onError(e);
        }
      });
      return this;
    };

    return Funclet;

  })();

  bind = function() {
    var args, func;
    func = arguments[0], args = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
    return Funclet.make().then(function(next) {
      return func.apply(null, __slice.call(args).concat([next]));
    });
  };

  start = function(func) {
    return Funclet.make().then(func);
  };

  map = function(ary, proc) {
    return Funclet.make().map(ary, proc);
  };

  mapSeries = function(ary, proc) {
    return Funclet.make().mapSeries(ary, proc);
  };

  each = function(ary, proc) {
    return Funclet.make().each(ary, proc);
  };

  eachSeries = function(ary, proc) {
    return Funclet.make().eachSeries(ary, proc);
  };

  parallel = function(tasks) {
    return Funclet.make().parallel(tasks);
  };

  module.exports = {
    start: start,
    bind: bind,
    map: map,
    mapSeries: mapSeries,
    each: each,
    eachSeries: eachSeries,
    parallel: parallel
  };

}).call(this);
